...
class RealtorspiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        if "realestateandhomes-detail" in self.url:
            ...
            yield scrapy.Request(self.url, headers=headers, meta=meta)
        elif "realestateagents" in self.url:
            self.logger.info("in agent page = {}".format(self.url))
            yield scrapy.Request(
                self.url, callback=self.parse_agent, headers=headers, meta=meta
            )
        else:
    ...
    def parse_state(self, response):
        ...
        try:
            links = [
                "https://www.realtor.com/realestateandhomes-detail/{}/".format(
                    item["permalink"]
                )
                for item in data["props"]["pageProps"]["properties"]
            ]
        except KeyError:
            try:
                links = [
                    "https://www.realtor.com{}/".format(atags.attrs["href"])
                    for atags in soup.css.select(
                        'div.result-list li[data-testid="result-card"] a'
                    )
                ]
            except KeyError:
                pass
        ...
        for link in links:
            ...
            yield scrapy.Request(link, headers=headers, meta=meta)

    def parse(self, response):
        self.logger.info("Parse property function called on %s", response.url)
        soup = BeautifulSoup(response.text, "html.parser")
        try:
            item = json.loads(soup.css.select("script#__NEXT_DATA__")[0].text)["props"][
                "pageProps"
            ]["property"]
        except:
            item = json.loads(soup.css.select("script#__NEXT_DATA__")[0].text)["props"][
                "pageProps"
            ]["initialReduxState"]["propertyDetails"]
        ...
        for advertiser in item["consumer_advertisers"]:
            if advertiser["type"] == "Agent" and advertiser["href"]:
                ...
                yield scrapy.Request(
                    f"https://www.realtor.com{advertiser['href']}",
                    headers=headers,
